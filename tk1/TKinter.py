...
from tkinter import *
import easygui,time,MYSQL,json,EXE_SSH
class MY_GUI():

    def __init__(self,xiaoxue,IP):
        self.xiaoxue=xiaoxue
        self.IP=IP.strip()
        self.TITLE = "正在修改服务器 {}".format(self.IP)
    def XIAOXUE(self):
...
#标签
        self.label1=Label(self.xiaoxue,text="修改的站点(必填！！) :",bg="lightblue",height=1)
        self.label1.grid(row=0,column=0, sticky=W)          #行 列 左粘
        self.label2=Label(self.xiaoxue,text="修改为域名 :",bg="lightblue",height=1)
...
#输入框
 ...
 #      pass
#按钮
        self.button1=Button(self.xiaoxue,bg="pink",text="站点里的域名", activebackground='lime', command=self.BTN5, width=10)  #activebackground按下去颜色
...
#BTN5
    def BTN5(self):
        sh = self.entry1.get()
        sh = sh.strip()
        shh="cat /usr/share/nginx/html/{}/a.txt".format(sh)
        b = EXE_SSH.shell(shh, self.IP)  # 执行shell
        self.text1.insert(1.0, b)  # 输出到 listbox
        self.text1.insert(1.0, "-------------\n{}\n---------------  {} 站点下的域名是  ------------\n".format(self.TIME(),sh))
    ...
    def BTN6(self):         #sed替换按钮
        ...
        for a in aa:
            print(a)
            MYSQL.insert(self.IP, self.TIME(), a)           # 执行的命令添加在数据库
                      # 执行的命令写入json
...
#BTN7
    def SHELL(self):
        sh=self.entry3.get()
        sh=sh.strip()
        b=EXE_SSH.shell(sh,self.IP)                #执行shell
        self.text1.insert(1.0, b)  # 输出到 listbox
        self.text1.insert(1.0, "------------------\n{}\n------------------\n".format(self.TIME()))

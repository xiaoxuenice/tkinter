...
class MY_GUI():
    ...
    def __init__(self,xiaoxue,IP):
        ...
        self.ptym=''
        self.IP=IP.strip()
        self.TITLE = "NANNAN 服务器 {}".format(self.IP)
    def XIAOXUE(self):
...
###站点下拉框
        self.zhandian=['3659','pj8','xpj','wns666','wns888','bodog','bwin','yinhe']
...
#BTN7
    def SHELL(self):
        xym=self.entry3.get()
        xym=xym.strip()
        if re.findall('http|https',xym):
            pass
        else:
            easygui.msgbox("新域名必须添加http或https","错误")
            return 0              #执行shell
        ax=self.ptym.strip()
        bx=xym.strip()
